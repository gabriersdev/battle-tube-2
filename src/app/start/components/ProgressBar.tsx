...
export const ProgressBar: React.FC = () => {
  const {currentScreenIndex, isPlaying, remainingTime, currentScreen} = usePresentation();
  ...
  useEffect(() => {
    if (isPast) {
      ...
      if (isPlaying) {
        ...
        // O ponto atual é calculado baseando-se no tempo que falta
        const initialProgress = 1 - (remainingTime / duration);
        ...
        controls.start({
          width: '100%',
          transition: {
            duration: remainingTime / 1000, // Converte ms para segundos para o Framer Motion
            ease: 'linear'
          }
        });
      } else {
        // Se está pausado, para a animação no ponto atual
